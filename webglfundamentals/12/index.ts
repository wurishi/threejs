...
(() => {
  ...
  const u_fudgeFactor = webglUtils.getUniformLocation(
    gl,
    program,
    'u_fudgeFactor'
  );

  a_position.setFloat32(new Float32Array(geometry), 3);
  ...
  gui.add(api, 'fudge', 0, 2).onChange((f) => {
    fudgeFactor = f;
    drawScene();
  });
  ...
  function drawScene() {
    ...
    u_fudgeFactor.uniform1f(fudgeFactor);
