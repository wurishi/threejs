...
function main() {
  ...
  if (!gl) return;
  let f = fs.replace('{precision}', precision);
  f = f.replace('{fragment}', fragment);
  const program = webglUtils.createProgram2(gl, vs, f);
  ...
  function setMousePosition(e: { clientX: number; clientY: number }) {
  ...
  function render(now: number) {
    ...
    iResolution.uniform2f(canvas.width, canvas.height);
    iMouse.uniform2f(mouseX, mouseY);
